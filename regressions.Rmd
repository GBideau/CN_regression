...
```{r regression_logistique, warning=FALSE, eval=FALSE}
...
variables_sbs <- c(colnames(df_reg[which(stringr::str_detect(colnames(df_reg), "_quantile") == TRUE)]),
...
a_retirer <- row.names(nbr_na)[which(nbr_na$`apply(df_autocor, MARGIN = 2, function(x) {     sum(is.na(x)) })` > 1600)]
